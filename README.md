...
Everything renders offline — there are **no external image dependencies**. Every
game-card illustration, the hero scene and all UI icons are hand-built **inline SVG**
(one consistent outline icon set in `index.html`, game/hero scenes in
`assets/js/art.js`) plus CSS gradients. The only remote asset is the web font.
...
```
index.html             # markup / app shell + inline SVG icon sprite
assets/css/styles.css  # brand tokens, Bebas type system, components, motion
assets/js/art.js       # inline SVG art library (per-game scenes + hero)
